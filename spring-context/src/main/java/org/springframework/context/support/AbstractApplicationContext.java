...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	//修改bean定义信息的集合 -> 用来存放beanFactory的后置处理器集合
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			 ...
			 * 2.设置容器关闭标志位为false
			 * 3.设置容器激活标志位为true
			 * 4.获取Environment对象，校验相关的属性是否存在于环境
	...
	protected void prepareRefresh() {
		...
		// 此方法为空，目的是留给子类去实现，重写此方法，初始化属性资源
		initPropertySources();

		// Validate that all properties marked as required are resolvable:
		// see ConfigurablePropertyResolver#setRequiredProperties
		// 获取环境对象（环境对象在之前已经创建过），验证需要的属性文件是否都已放入环境中
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 设置beanfactory的表达式语言处理器（el表达式） 处理类（解析类（配置类））
		beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver(beanFactory.getBeanClassLoader()));
		// 为beanFactory增加一个默认的propertyEditor，这个主要是对bean的属性等设置管理的一个工具类 举例 -> name_age_gender 注册到Student对象中
		...
		// 所以在使用autowire进行注入的时候需要将这些接口进行忽略 --> ApplicationContextAwareProcessor已经设置过了一边，忽略下面的接口是为了防止重新设置两遍
	...
	protected void initApplicationEventMulticaster() {
		//获取当前bean工厂，一般是DefaulistableBeanFactory
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
...
//			getApplicationEventMulticaster().addApplicationListener(this.getBean(listenerBeanName,ApplicationListener.class));
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 如果beanFactory之前没有注册嵌入值解析器，则注册默认的嵌入值解析器，主要用于注解属性值的解析 -> 值解析器embeddedValueResolver的添加步骤在PropertySourcesPlaceholderConfigurer中有
		if (!beanFactory.hasEmbeddedValueResolver()) {
			beanFactory.addEmbeddedValueResolver(strVal -> getEnvironment().resolvePlaceholders(strVal));
		}

		// Initialize LoadTimeWeaverAware beans early to allow for registering their transformers early.
		// 尽早初始化loadTimeWeaverAware bean,以便尽早注册它们的转换器 -> aop相关
		...
		// 在进行实例化之前冻结所有的bean定义，说明注册的bean定义将不被修改或任何进一步的处理
